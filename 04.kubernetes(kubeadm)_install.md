...
- Vagrant(베이그런트)는 가상화(Virtualization)된 개발 환경을 만들고 관리하는 오픈소스 도구
- 설정 스크립트 파일(Vagrantfile)을 기반으로 특정 환경의 가상머신을 생성. 
- VirtualBox, VMware, Docker 등 다양한 가상화 소프트웨어와 연동되어, 필요한 애플리케이션이 설치된 경량의 재현 가능한 가상 머신을 명령어 몇 줄로 생성하고 관리할 수 있게 해 생산성을 높여줍니다
...
| vagrant up              | Vagrantfile을 읽어 Provisioning 진행(vm을 만들어주고 실행). <br> 이미 설치된 상태에서 실행만 함 |
...
>Provisioning
>사용자의 요구에 맞게 시스템 자원을 할당, 배치, 배포하여 필요 시 시스템을 즉시 사용할 수 있는 상태로 미리 준비
