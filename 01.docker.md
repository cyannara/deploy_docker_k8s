...
# Docker 패키지 저장소의 GPG 공개키를 다운로드
sudo curl -fsSL https://download.docker.com/linux/ubuntu/gpg -o /etc/apt/keyrings/docker.asc

# 파일을 모든 사용자가 읽을 수 있도록 권한을 변경
...
컨테이너 명령                 생략형
  ...
  docker container prune   =  docker prune     <= 중지된 컨테이너를 전부 삭제
...
`실행 중이거나 중지된 모든 컨테이너를 강제 삭제`  

```
docker rm -f `docker ps -a -q`
  # docker : 도커 클라이언트
  # rm     : 컨테이너 삭제
  # -f     : 실행중인 컨테이너도 강제 종료 후 삭제
  # ``     : 명령 치환. 안쪽 명령의 출력 결과를 바깥 명령의 인자로 넘겨줌
  # ps     : 컨테이너 목록조회
  # -a     : 중지된 컨테이너도 포함
  # -q     : ID만 출력(quiet mode)
  # 컨테이너가 하다도 없으면 에러가 남.
...
docker system prune     # 위 전부 한번에 정리
```
...
sudo docker run -d -p 3306:3306 -e MYSQL_ROOT_PASSWORD=admin -e TZ=Asia/Seoul -v vtest:/var/lib/mysql --name mysql  mysql:8.0
...
#   --name : 컨테이너 이름 지정
```
c드라이브에 'mysql_data' 폴더를 만든 경우   `-v /mnt/c/mysql_data:/var/lib/mysql` 로 매핑이 하면 됨.

■ CLI(터미널)에서 긴 명령어를 줄바꿈하여 입력하는 방법은 사용하는 쉘(shell) 환경에 따라 다름
